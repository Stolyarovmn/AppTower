...
import { validate, flatten, defaults } from './core/model.js';
...
const sections = {
  ...
  about: 'О программе AppTower',
};
function groupSince(before, title) {
  const nodes = [...content.children].slice(before);
  const group = document.createElement('section');
  group.className = 'settings-group';
  group.setAttribute('aria-label', title);
  group.append(...nodes); content.append(group); return group;
}
...
function actionButton(title, fn, options = {}) {
  ...
  const name = options.icon || ({
    ...
    'Переименовать':'edit', 'Удалить':'trash', 'Сбросить':'sync',
...
function render() {
  ...
  nav.replaceChildren(
    ...Object.entries(sections).map(([id, label]) => {
      ...
      b.insertAdjacentHTML('afterbegin', icon(({general:'settings',appearance:'palette',workspaces:'workspaces',shortcuts:'group',recent:'clock',sites:'globe',performance:'performance',modules:'modules',apps:'app',data:'sync',diagnostics:'search',reset:'sync',about:'info'})[id]));
      b.dataset.section = id;
      b.setAttribute('aria-current', section === id ? 'page' : 'false');
      return b;
    }),
  );
  for (const [title, ids] of [['Интерфейс',['general','appearance']],['Организация',['workspaces','shortcuts','recent']],['Сайты и ресурсы',['sites','performance','modules','apps']],['Обслуживание',['data','diagnostics','reset','about']]]) {
  ...
  if (section === 'appearance') {
    ...
    card('Общий вид', []);
    const segments = document.createElement('div'); segments.className='theme-segments';
    for (const [theme,label,glyph] of [['system','Система','monitor'],['light','Светлая','sun'],['dark','Тёмная','moon']]) {
      const b = actionButton(label,()=>act({type:'settings',value:{theme}}),{icon:glyph});
      b.setAttribute('aria-pressed',String(state.settings.theme===theme)); segments.append(b);
    }
    content.append(segments); groupSince(start,'Общий вид');
    const colorStart=content.children.length;
    ...
    card('Цвет акцента', [
      ['Сбросить', () => act({ type: 'settings', value: { accent: '' } }),{icon:'sync'}],
    ]);
    const palette=document.createElement('div');palette.className='accent-palette';
    for (const value of ['#8896aa','#a697bf','#bfa0aa','#c6a68c','#c2b78e','#9aaf96','#8bafad','#83a6bf']) {
      const b=button('',()=>act({type:'settings',value:{accent:value}}));
      b.className='accent-swatch';b.style.background=value;b.title=value;b.setAttribute('aria-label',value);
      b.setAttribute('aria-pressed',String(state.settings.accent===value));palette.append(b);
    }
    palette.append(customColorControl(color));content.append(palette);
    groupSince(colorStart,'Цветовая тема');
    row(
  ...
  if (section === 'sites') {
    ...
    for (const origin of origins) {
      ...
      groupSince(start,origin).classList.add('site-settings-group');
    }
  }
  if (section === 'modules') {
  ...
  if (section === 'reset') {
    hint('Восстановить оформление и параметры производительности. Ярлыки, рабочие области, модули и правила сайтов сохранятся.');
    card('Параметры AppTower', [['Сбросить',()=>{
      if(confirm('Восстановить оформление и производительность по умолчанию?'))
        return act({type:'settings',value:{...defaults().settings,sync:state.settings.sync}});
    },{icon:'sync'}]]);
  }
  if (section === 'about') {
    card('AppTower '+chrome.runtime.getManifest().version,[]);
    hint('Рабочие области, группы и шаблоны сайтов в боковой панели браузера.');
    card('Обратная связь', [['Сообщить о проблеме',()=>chrome.tabs.create({url:'https://github.com/Stolyarovmn/AppTower/issues/new'}),{icon:'external'}]]);
  }
...
const searchBox=document.createElement('input');searchBox.type='search';searchBox.placeholder='Поиск настроек';searchBox.setAttribute('aria-label','Поиск настроек');searchBox.className='settings-search';
document.getElementById('title').before(searchBox);
searchBox.oninput=()=>{
  const q=searchBox.value.trim().toLocaleLowerCase();
  if(!q){render();return;}
  content.replaceChildren();
  const terms={appearance:'тема цвет светлая темная перекрытие',sites:'масштаб уведомления усыплять окно',data:'импорт экспорт синхронизация',modules:'youtube модуль',reset:'сброс восстановить'};
  for(const [id,title] of Object.entries(sections))if((title+' '+(terms[id]||'')).toLocaleLowerCase().includes(q))
    card(title,[['Открыть',()=>{section=id;location.hash=id;searchBox.value='';render();},{icon:'external',iconOnly:true}]]);
  if(!content.children.length)hint('Ничего не найдено');
};
render();
