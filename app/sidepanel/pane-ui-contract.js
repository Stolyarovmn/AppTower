const ICONS={
  more:'<svg viewBox="0 0 20 20" aria-hidden="true"><circle cx="4" cy="10" r="1.15"/><circle cx="10" cy="10" r="1.15"/><circle cx="16" cy="10" r="1.15"/></svg>',
...
function installStyle(){
  ...
  style.textContent=`
    .pane-toolbar{gap:4px!important;padding:6px 8px!important}
    .pane-toolbar .url-input{min-width:92px!important;flex:1 1 150px!important}
    ...
    /* One visual language: every toolbar action is a plain 30x30 icon button.
       No framed tiles, no persistent outline, no button-specific chrome. */
    .pane-toolbar .atn-pane-more,
    .pane-toolbar .atn-pane-close{
      flex:0 0 30px!important;width:30px!important;height:30px!important;
      display:grid;place-items:center;padding:0!important;border:0!important;
      border-radius:6px!important;background:transparent!important;color:inherit!important;
      box-shadow:none!important;outline:none!important;
    }
    .pane-toolbar .atn-pane-more:hover,
    .pane-toolbar .atn-pane-close:hover{background:var(--hover)!important}
    .pane-toolbar .atn-pane-more:focus-visible,
    .pane-toolbar .atn-pane-close:focus-visible{outline:1px solid var(--accent)!important;outline-offset:-2px!important}
    .pane-toolbar .atn-pane-more svg,
    .pane-toolbar .atn-pane-close svg{width:18px;height:18px;fill:none;stroke:currentColor;stroke-width:1.55;stroke-linecap:round;stroke-linejoin:round}
    .pane-toolbar .atn-pane-more svg circle{fill:currentColor;stroke:none}

    /* Closing a pane only makes sense in split mode. Hiding X in single-pane
       mode also keeps the primary toolbar visually quiet. */
    .pane-toolbar .atn-pane-close{display:none}
    #workspace[data-layout="split"] .pane-toolbar .atn-pane-close{display:grid}

    .atn-pane-menu{position:fixed;z-index:2147483646;min-width:220px;padding:6px;border-radius:9px;border:1px solid var(--border,rgba(255,255,255,.14));background:var(--panel,#292929);box-shadow:0 8px 28px rgba(0,0,0,.32)}
    .atn-pane-menu[hidden]{display:none}
    .atn-pane-menu button{width:100%;min-height:34px;padding:7px 10px;border:0;border-radius:6px;background:transparent;color:inherit;text-align:left;font:inherit;cursor:pointer}
    .atn-pane-menu button:hover,.atn-pane-menu button:focus-visible{background:var(--hover,rgba(255,255,255,.10));outline:none}
...
function positionMenu(menu,button){
  ...
  const top=Math.max(gutter,Math.min(window.innerHeight-mr.height-gutter,r.bottom+5));
...
function installPane(pane){
  ...
  more.addEventListener("click",event=>{
    ...
    sync();
    if(menu.hidden){positionMenu(menu,more);more.blur();}
