...
test('actual Side Panel boots, adds browser tab, searches, splits and updates metadata without frame reload',async()=>{
  ...
  globalThis.chrome={windows:{getCurrent:async()=>({id:1})},storage:{session:{get:async()=>({}),set:async()=>{}},onChanged:changed},runtime:{getURL:path=>'chrome-extension://test'+path,onMessage:event(),connect:()=>({onMessage:event(),disconnect(){}}),sendMessage:async m=>{messages.push(m);if(m.type==='APP_GET')return {ok:true,state:structuredClone(state)};if(m.type==='GET_ACTIVE_TAB')return {ok:true,tab:{title:'Browser tab',url:'https://browser.test/page'}};if(m.type==='APP_MUTATE'){state=reduce(state,m.action);changed.emit({[KEY]:{newValue:structuredClone(state)}},'local');return {ok:true,state:structuredClone(state)};}return {ok:true};}},tabs:{create:async()=>({})}};
  ...
  $('add').click();await tick();const dialog=document.querySelector('dialog[open]');assert.ok(dialog);const inputs=dialog.querySelectorAll('input');assert.equal(inputs[1].value,'https://browser.test/page');dialog.querySelector('form').dispatchEvent(new dom.window.Event('submit',{cancelable:true}));await tick();
  assert.equal(state.workspaces[0].items.length,1);
  $('shortcut-list').querySelector('button').click();await tick();
  const top=document.querySelector('.pane[data-pane="top"] iframe');assert.equal(top.src,'https://browser.test/page');const count=srcWrites.filter(x=>x.frame===top).length;
