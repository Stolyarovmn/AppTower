...
test('transparent favicon has no fallback letters underneath; failed icon restores initials',()=>{
 ...
 const img=tile.querySelector('img');assert.equal(tile.textContent,'EX');img.dispatchEvent(new dom.window.Event('load'));assert.equal(tile.textContent,'');assert.equal(tile.querySelector('img'),img);
