...
function apiCapabilities(){
  return {
    ...
    onOpened:Boolean(chrome.sidePanel?.onOpened),
...
function compact(value){
  ...
  if(typeof value==="object"){
    const out={};
    for(const [key,item] of Object.entries(value).slice(0,40)) out[key]=compact(item);
    return out;
  }
  return value;
}
...
function devLog(event,payload={}){
  const entry={ts:new Date().toISOString(),event,payload:compact(payload)};
  console.info("[ATV2]",event,entry.payload);
  logWrite=logWrite.then(async()=>{
    const current=(await chrome.storage.local.get(LOG_KEY))[LOG_KEY];
  ...
  }).catch(error=>console.warn("[ATV2] log write failed",error));
  return entry;
}
...
function setLifecycle(windowId,event){
  const before=lifecycle(windowId);
  const next=reduceLifecycle(before,event);
  windows.set(windowId,next);
  devLog("lifecycle.transition",{windowId,event:event?.type,from:before.phase,to:next.phase,pendingCommand:next.pendingCommand?.type||null});
  syncRail(windowId);
  return next;
}

function addPort(map,windowId,port,kind){
  if(!map.has(windowId)) map.set(windowId,new Set());
  map.get(windowId).add(port);
  devLog("port.connected",{kind,windowId,count:map.get(windowId).size});
  port.onDisconnect.addListener(()=>{
    const set=map.get(windowId);
    set?.delete(port);
    if(set && !set.size) map.delete(windowId);
    devLog("port.disconnected",{kind,windowId,count:map.get(windowId)?.size||0});
  });
}
...
function queueCommand(windowId,command){
  devLog("command.queued",{windowId,command:command?.type||null});
...
function deliverPending(windowId){
  ...
  if(!ports.length){devLog("command.waiting-for-panel",{windowId,command:state.pendingCommand?.type||null});return;}
  devLog("command.delivered",{windowId,command:state.pendingCommand?.type||null,ports:ports.length});
...
async function openWindowPanel(windowId,command=null){
  devLog("panel.open.request",{windowId,command:command?.type||null,state:lifecycle(windowId).phase,api:apiCapabilities()});
  ...
  if(state.phase===PHASE.EXPANDED && panelPorts.get(windowId)?.size){
    ...
    devLog("panel.open.reused",{windowId});
    return {ok:true,reused:true};
  }
  if(state.phase===PHASE.OPENING) return {ok:true,pending:true};
  if(state.phase===PHASE.CLOSING) return {ok:false,busy:true,error:"AppTower ещё завершает сворачивание. Повторите нажатие после завершения."};

  setLifecycle(windowId,{type:"OPEN_REQUEST",command:command||state.pendingCommand});
  try{
    await chrome.sidePanel.open({windowId});
    devLog("panel.open.api-resolved",{windowId});
    return {ok:true,reused:false};
  }catch(error){
    setLifecycle(windowId,{type:"OPEN_FAILED"});
    devLog("panel.open.failed",{windowId,error:String(error?.message||error)});
...
async function prepareRailForCollapse(windowId){
  ...
  devLog("collapse.rail.response",{windowId,response});
...
async function collapseWindowPanel(windowId){
  ...
  devLog("panel.collapse.request",{windowId,state:state.phase,api});
  if(state.phase===PHASE.CLOSING) return {ok:true,pending:true};
  if(!chrome.sidePanel?.close){
    const error="Этот Edge не предоставляет безопасный window-scoped sidePanel.close(). AppTower не будет использовать tab-specific workaround.";
    devLog("panel.collapse.unsupported",{windowId,error,api});
  ...
  try{
    ...
    devLog("panel.close.api-resolved",{windowId});
    if(!chrome.sidePanel?.onClosed) setLifecycle(windowId,{type:"PANEL_CLOSED"});
    return {ok:true};
  }catch(error){
    setLifecycle(windowId,{type:"CLOSE_FAILED"});
    devLog("panel.collapse.failed",{windowId,error:String(error?.message||error)});
    return {ok:false,error:String(error?.message||error)};
  }
}
...
async function diagnosticsReport(){
  await logWrite;
  const stored=await chrome.storage.local.get([LOG_KEY,"atv2.data.v1"]);
  const winList=[];
  for(const [windowId,state] of windows) winList.push({windowId,state,panelPorts:panelPorts.get(windowId)?.size||0,railPorts:railPorts.get(windowId)?.size||0});
  const browserWindows=await chrome.windows.getAll({populate:true}).catch(()=>[]);
  return {
    ...
    api:apiCapabilities(),
    lifecycle:winList,
    browserWindows:browserWindows.map(win=>({id:win.id,focused:win.focused,state:win.state,tabs:(win.tabs||[]).map(tab=>({id:tab.id,active:tab.active,url:tab.url||""}))})),
    appState:stored["atv2.data.v1"]||null,
...
async function runSelfTest(){
  const api=apiCapabilities();
  const windowsNow=await chrome.windows.getAll({populate:true}).catch(()=>[]);
  const checks=[
    {name:"sidePanel API exists",pass:api.sidePanel},
    {name:"sidePanel.open exists",pass:api.open},
    {name:"sidePanel.close exists",pass:api.close},
    {name:"sidePanel.onOpened exists",pass:api.onOpened},
    {name:"sidePanel.onClosed exists",pass:api.onClosed},
    {name:"no tab-scoped lifecycle in Cleanroom",pass:true},
    {name:"browser window visible to extension",pass:windowsNow.length>0}
  ];
  const result={generatedAt:new Date().toISOString(),api,checks,pass:checks.every(item=>item.pass)};
  devLog("diagnostics.selftest",result);
  return result;
}
...
chrome.runtime.onConnect.addListener(port=>{
  if(port.name==="ATV2_RAIL"){
    ...
    addPort(railPorts,windowId,port,"rail");
    const state=lifecycle(windowId);
    if(state.phase===PHASE.UNKNOWN) void settleUnknown(windowId); else syncRail(windowId);
    return;
  }
  const match=/^ATV2_PANEL:(\d+)$/.exec(port.name||"");
  if(match){
    const windowId=Number(match[1]);
    addPort(panelPorts,windowId,port,"panel");
...
chrome.runtime.onMessage.addListener((message,sender,sendResponse)=>{
  ...
  if(message.type==="DEV_LOG"){devLog(`ui.${message.event||"event"}`,{...message.payload,senderWindowId});sendResponse({ok:true});return;}
  if(message.type==="OPEN_DIAGNOSTICS"){void chrome.tabs.create({url:chrome.runtime.getURL("diagnostics.html")}).then(()=>sendResponse({ok:true})).catch(error=>sendResponse({ok:false,error:String(error)}));return true;}
  if(message.type==="GET_DIAGNOSTICS"){void diagnosticsReport().then(report=>sendResponse({ok:true,report})).catch(error=>sendResponse({ok:false,error:String(error)}));return true;}
  if(message.type==="RUN_SELF_TEST"){void runSelfTest().then(result=>sendResponse({ok:true,result})).catch(error=>sendResponse({ok:false,error:String(error)}));return true;}
  if(message.type==="CLEAR_DIAGNOSTICS"){void chrome.storage.local.remove(LOG_KEY).then(()=>sendResponse({ok:true}));return true;}

  if(message.type==="OPEN_PANEL"){
    const windowId=Number.isInteger(Number(message.windowId))?Number(message.windowId):senderWindowId;
    if(!Number.isInteger(windowId)) return;
    void openWindowPanel(windowId,message.command||null).then(sendResponse);return true;
  }
  if(message.type==="COLLAPSE_PANEL"){
    const windowId=Number(message.windowId);
    if(!Number.isInteger(windowId)) return;
    void collapseWindowPanel(windowId).then(sendResponse);return true;
  }
  if(message.type==="GET_ACTIVE_TAB"){
    const windowId=Number(message.windowId);
    if(!Number.isInteger(windowId)) return;
    void chrome.tabs.query({active:true,windowId}).then(([tab])=>sendResponse({ok:true,tab:tab?{id:tab.id,title:tab.title||"",url:tab.url||""}:null})).catch(error=>sendResponse({ok:false,error:String(error)}));return true;
  }
});
