...
The current regression PR #2 head observed during this scan is `6ec579a64af0ba9d1346a04a0f3df854aaba839c`. Its `validate` workflow run `34061140457` completed with conclusion **`action_required`**, so CI is not green. READY TASKS remain blocked from activation until a successful validation run is observed. PR #1's previous head `e05c8e61c87269f776d13252791f1360d4d13e54` is the predecessor of this backlog update, but that does not override the newer regression-branch gate.
...
### TASK 3 — Deterministic drag/drop interaction model — 90/100 — BLOCKED
...
**Sources/competitors:** AppTower `docs/10_KNOWN_ISSUES.md` P0 Group/template drag regression. SuperSplit (`schappim/supersplit-js`, created 2026-06-26, MIT) provides current behavior evidence for explicit drag-to-re-split/drop-zone feedback; AppTower must independently implement its smaller interaction model and should not copy the tiling engine or source code.

### TASK 4 — Safe pane sleep guards — 89/100 — READY
...
### TASK 5 — Versioned persistence schema + append-only migrations — 88/100 — BLOCKED
...
### TASK 6 — Command Palette — 86/100 — BLOCKED
...
### TASK 7 — Event-driven resource budget scheduling — 85/100 — BLOCKED
...
### TASK 8 — Event-based workspace snapshots + Undo — 84/100 — BLOCKED
...
### TASK 9 — Installed-extension lifecycle E2E harness — 83/100 — BLOCKED
...
### TASK 10 — Compatibility ladder — 82/100 — BLOCKED
...
### TASK 11 — Manifest permission budget + CI regression gate — 81/100 — BLOCKED
...
### TASK 12 — Duplicate shortcut detection — 80/100 — READY
...
### TASK 13 — Context-scoped pane bridge/PWA content-script injection — 79/100 — BLOCKED
...
### TASK 14 — Restorable split layout metadata — 79/100 — BLOCKED
...
### TASK 15 — Native tab-group import/export bridge — 78/100 — BLOCKED
...
### TASK 16 — Glance preview in temporary bottom pane — 76/100 — BLOCKED
...
|---:|---:|---|---|---|
| 1 | 74 | Per-site sleep policy presets: default/aggressive/never sleep | Drowzy (MIT) | Promote after TASK 4 + measurable resource baseline |
...
| 3 | 73 | Resource-pressure-aware emergency eviction using coarse system-memory pressure, LRU and pane safety guards | TabRest (MIT) | Promote after TASK 4 + TASK 7 baseline; justify `system.memory`; no per-pane heap polling/broad host access |
...
| 6 | 72 | Workspace/session import from other managers | VertiTab, Lunma, Tabwise | Promote after TASK 5 export/import schema; avoid mandatory history permission |
